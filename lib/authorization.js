...
module.exports = {
  determineAuthorization: function (combinedOptions, handlerObject, req) {
    ...
    return co(function * () {
      ...
      if (cachedData && combinedOptions.cache) {
        ...
        if (type(combinedOptions.roles) === 'Array') {
          ...
          const res = combinedOptions.roles(handlerObject)
          if (res.then) {
            allowed = yield res
          } else {
            allowed = res
        ...
        if (type(handlerObject.roles) === 'Array') {
          ...
          const res = handlerObject.roles(handlerObject)
          if (res.then) {
            actual = yield res
          } else {
            actual = res
